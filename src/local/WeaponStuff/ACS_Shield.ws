...
state Aard_Pull in cACS_Shield_Summon
{
	...
	{	
		...
		thePlayer.PlayEffect('swarm_gathers');
	...
	latent function NPC_Fear_Start()
	{
		...
		for( i = 0; i < actors.Size(); i += 1 )
		{
			...
			{				
				...
				npc.PlayEffect('focus_sound_red_fx');
		...
		for (i = 0; i < entities.Size(); i += 1) 
		{
			...
			if (!npc.IsAlive() && thePlayer.IsInCombat()) 
			{
				...
				*/
				...
				rev_ent.PlayEffect('blood_spill');
				rev_ent.PlayEffect('blood_throat_cut');
				rev_ent.PlayEffect('yrden_shock');
				rev_ent.PlayEffect('yrden_slowdown');
				rev_ent.PlayEffect('critical_poison');
	...
	latent function Skele_Summon_Actual()
	{
		...
		for (i = 0; i < entities.Size(); i += 1) 
		{
			...
			if (!npc.IsAlive() && thePlayer.IsInCombat()) 
			{
				...
				skele_ent.PlayEffect('appear');
				skele_ent.StopEffect('appear');
				skele_ent.PlayEffect('explode');
				skele_ent.StopEffect('explode');
				skele_ent.PlayEffect('critical_burning_green');
				skele_ent.PlayEffect('yrden_shock');
				skele_ent.PlayEffect('yrden_slowdown');
...
state Quen_Wolf_Summon in cACS_Shield_Summon
{
	...
	{	
		...
		for( i = 0; i < wolfCount; i += 1 )
		{
			...
			wolf_ent.PlayEffect('appear');
			wolf_ent.StopEffect('appear');
			wolf_ent.PlayEffect('shadow_form');
			wolf_ent.PlayEffect('demonic_possession');
...
state Quen_Centipede_Summon in cACS_Shield_Summon
{
	...
	{	
		...
		for( i = 0; i < centipedeCount; i += 1 )
		{
			...
			centipede_ent.PlayEffect('shadow_form');
			centipede_ent.PlayEffect('demonic_possession');
...
state Axii_Persistent_Shield in cACS_Shield_Summon
{
	...
	{	
		...
		if (thePlayer.HasTag('axii_sword_equipped'))
		{
			...
			//shield.PlayEffect('fire_sparks_trail');

			//shield.PlayEffect('runeword1_fire_trail');

			//shield.PlayEffect('appear');

			//shield.PlayEffect('burn');
			...
			shield.PlayEffect('destroy');

			shield.StopEffect('destroy');

			shield.PlayEffect('destroy_shield_fx');

			shield.StopEffect('destroy_shield_fx');

			shield.PlayEffect('igni_cone_hit');
			...
			//shield.PlayEffect('aard_cone_hit');
...
state Axii_Shield_Entity in cACS_Shield_Summon
{
	...
	{	
		...
		if (thePlayer.HasTag('axii_sword_equipped'))
		{
			...
			shield_entity.PlayEffect('hym_summon');
...
function ACS_Axii_Shield_Destroy_DELAY()
{
	...
	{
		ACS_Shield().PlayEffect('disappear_fast');
		//ACS_Shield().StopEffect('disappear');
		ACS_Shield().DestroyAfter(0.25);
	}
	else
	{
		ACS_Shield().PlayEffect('disappear');
		//ACS_Shield().StopEffect('disappear');
...
function shield_entity_despawn_anim()
{
	...
	ACS_Shield_Entity().PlayEffect('hym_despawn');
...
function ACS_Revenant_Destroy()
{
	...
	for( i = 0; i < revenant.Size(); i += 1 )
	{
		...
		revenant[i].PlayEffect('demonic_possession');
...
function ACS_Skele_Destroy()
{
	...
	for( i = 0; i < skeleton.Size(); i += 1 )
	{
		...
		skeleton[i].PlayEffect('death_glow');
		skeleton[i].PlayEffect('suck_into_painting');
...
function ACS_Centipede_Destroy()
{
	...
	for( i = 0; i < centipedes.Size(); i += 1 )
	{
		...
		centipedes[i].PlayEffect('dirt_jump');
